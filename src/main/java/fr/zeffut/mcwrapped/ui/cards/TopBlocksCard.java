...
public final class TopBlocksCard implements Card {
    ...
    public void tick(final int width, final int height) {
        ...
        for (int i = 0; i < top.size(); i++) {
            if (ticks == FIRST_ROW_START + i * ROW_STAGGER) {
                WrappedSounds.play(MinecraftClient.getInstance(), SoundEvents.BLOCK_STONE_BREAK, 1.0f + i * 0.3f, 0.5f);
            }
        }
    }
    ...
    private void renderEmpty(final DrawContext ctx, final TextRenderer tr, final int width, final int height, final float now) {
        ...
        ctx.drawCenteredTextWithShadow(tr, Text.literal("nothing mined this month"), width / 2, height / 2, color);
    }
    ...
    private static ItemStack stackFor(final String blockId) {
        final Identifier id = Identifier.tryParse(blockId);
    ...
    private static Text blockName(final String id) {
        final String[] parts = id.split(":");
        if (parts.length != 2) return Text.literal(prettify(id));
        return Text.translatable("block." + parts[0] + "." + parts[1]);
    }
